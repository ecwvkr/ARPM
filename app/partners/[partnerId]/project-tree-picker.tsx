...
import { getProjectSubtree } from "@/app/actions/projects";
...
export function ProjectTreePicker({
  ...
  onChange: (grants: { projectId: string; includeSubtree: boolean }[]) => void;
}) {
  const [tree, setTree] = useState<TreeNode | null>(null);
  const [state, setState] = useState<GrantState>({});

  useEffect(() => {
    getProjectSubtree(rootProjectId).then((t) => {
      setTree(t);
      if (t) {
  ...
  function toGrants(s: GrantState) {
    return Object.entries(s)
      .filter(([, v]) => v.checked)
      .map(([projectId, v]) => ({ projectId, includeSubtree: v.includeSubtree }));
  }
  ...
  return (
    <div className="space-y-1.5">
      ...
      <p className="text-xs text-muted-foreground">
        하위 노드를 체크 후 &apos;이 프로젝트만&apos;로 지정하면 그 아래 가지는 접근에서 제외됩니다.
      </p>
